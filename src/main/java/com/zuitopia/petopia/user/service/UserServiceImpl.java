...
public class UserServiceImpl implements UserService{
    ...
    public String signUpUser(UserRequestDTO userRequestDTO) throws Exception {
        try{
            // users 테이블에 새로운 유저를 생성하기
            int insertResult = userMapper.insertUser(userRequestDTO);

            if(insertResult < 1)
                throw new Exception("유저가 생성되지 않았습니다");

            // userId 할당받기
            int userId = userMapper.getUserIdByEmail(userRequestDTO.getUserEmail());
    ...
    public String loginUser(UserRequestDTO userRequestDTO) throws Exception {
        ...
        if (userSecurityVO != null && passwordEncoder.matches(userRequestDTO.getPassword(), userSecurityVO.getPassword())) {
